...
class QuranRepository(private val quranDao: QuranDao) {
  ...
  fun getAyahsForSurah(surahNumber: Int): List<Ayah> {
    return QuranData.getAyahsForSurah(surahNumber)
  }
  ...
  fun searchQuran(query: String): List<Pair<Surah, Ayah?>> {
    ...
    val allLoadedAyahs = QuranData.alIsraAyahs + QuranData.alKahfAyahs + QuranData.alFatihahAyahs + QuranData.anNabaAyahs
